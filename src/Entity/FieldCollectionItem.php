...
class FieldCollectionItem extends ContentEntityBase implements FieldCollectionItemInterface {
  ...
  public function getHostId() {
    if (!isset($this->host_id)) {
      if ($this->id() && $this->host_type->value) {
        ...
        if (Database::getConnection()->schema()->tableExists($table)) {
          ...
          $host_id_results = \Drupal::database()->query('SELECT `entity_id` FROM {' . $table . '} ' . 'WHERE `' . $this->bundle() . '_target_id` = ' . $this->id())->fetchCol();
  ...
  /**
   * Gets the entity type id of the host.
   *
   * @return string
   *   The host entity type id.
   */
  protected function getHostType() {
    $field = $this->getFieldDefinition($this->bundle());
